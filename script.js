/*
  SCRIPT.JS
  This file governs the entire interactive experience of the Kindred AI application.
  It is organized into distinct modules for clarity:
  1. DYNAMIC CLOUD BACKGROUND: Manages the procedural generation and rendering of the interactive cloud animation on the HTML5 canvas.
  2. FORM & UI LOGIC: Handles the multi-step user onboarding form, state management, and dynamic rendering of UI components.
  3. API INTERACTION: Contains the logic for communicating with the Google Generative AI API to fetch the personalized welcome message.
  4. BLUEPRINT GENERATION: Constructs the final AI instruction set based on user input and language selection.
*/

document.addEventListener('DOMContentLoaded', () => {
    // --- 1. DYNAMIC CLOUD BACKGROUND ---
    const canvas = document.getElementById('cloud-canvas');
    const ctx = canvas.getContext('2d');
    let mouse = { x: window.innerWidth / 2, y: window.innerHeight / 2 };
    let time = 0;

    // A self-invoking function to create a closure for the Perlin noise algorithm.
    // This is a classic algorithm for generating natural-looking procedural textures.
    const noise = (() => {
    ...
    // The main animation loop for the cloud background.
    function drawClouds() {
        const width = canvas.width, height = canvas.height;

        // Guard clause to prevent errors if the canvas has no size.
        if (width <= 0 || height <= 0) {
        ...
        // Iterate over every pixel, calculate its color and alpha based on noise and mouse position.
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                const dx = x - mouse.x, dy = y - mouse.y;
                const dist = Math.sqrt(dx * dx + dy * dy);
                // The "wind" effect: pixels near the cursor are pushed away.
                const repelStrength = Math.max(0, 1 - dist / 300);
                const xOffset = x + repelStrength * dx * 0.5, yOffset = y + repelStrength * dy * 0.5;
                
                // Layering multiple noise functions (fractal noise) creates a more realistic cloud texture.
                ...
                const i = (y * width + x) * 4;
                data[i] = 173; data[i + 1] = 216; data[i + 2] = 230; data[i + 3] = alpha; // Light blue clouds
            }
        }
    ...
    // Configuration object for all form steps. This makes the form easily extensible.
    const stepsConfig = [
    ...
    // Blueprint text, translated into multiple languages.
    ...
    // Manages visibility of form steps.
    ...
    // Advances the form to the next step or triggers the final generation.
    ...
    // Main event listener to begin the journey.
    startBtn.addEventListener('click', () => {
        introContainer.style.display = 'none';
        // Create and append all form step elements to the DOM.
        stepsConfig.forEach(config => {
            ...
            if (config.type === 'contentment-bar') {
                inputEl.addEventListener('click', e => {
                    ...
                    if (option) { 
                        userProfile[config.id] = option.dataset.value; 
                        advanceToNextStep(); 
                    }
                });
            } else {
                ...
                inputEl.addEventListener('keydown', e => {
                    if (e.key === 'Enter') { 
                        e.preventDefault(); 
                        if (e.target.value.trim()) advanceToNextStep(); 
                    }
                });
                inputEl.addEventListener('blur', e => userProfile[config.id] = e.target.value);
            }
        });
        advanceToNextStep(); // Show the first step.
    });

    // --- 3. API INTERACTION ---

    // Displays a loading message, calls the AI API, and then shows the final output.
    ...
    // Fetches the personalized welcome message from the Gemini API.
    async function getAiHello() {
        ...
        // IMPORTANT: In a real-world application, the API key should be handled securely on a server,
        // not exposed in the client-side code. This is a placeholder for this prototype.
        const apiKey = ""; 
    ...
    // --- 4. BLUEPRINT GENERATION & FINAL UI ---

    // Renders the final output screen with the AI message and blueprint.
    function showFinalOutput() {
        ...
        // Populate the final elements with data and add event listeners.
    ...
    // Generates the complete blueprint text by combining the user profile with the translated template.
